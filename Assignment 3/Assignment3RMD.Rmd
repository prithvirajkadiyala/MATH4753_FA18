...
###c
```
We can see that the lead has 99% confidence interval between `r leadci$conf.int` whereas copper has a 99% confidence interval between `r copperci$conf.int` From this we can say lead has more distribution around its mean when compared to the copper.
```
###d
```
...
```
So, our 95% difference confidence interval is `r diffci$conf.int`
```
